  ...
  try
    ...
    try
      ...
      begin
        ...
        for var M in Doc.Methods do Msg := Msg + M.Name + ', ';
        Assert.AreEqual(4, Doc.Methods.Count, Msg);
      end;
      
      // Look for specific method
      Method := nil;
      for var M in Doc.Methods do if M.Name = 'MethodTwo' then Method := M;
      ...
      // Check SyncMode
      Method := nil;
      for var M in Doc.Methods do if M.Name = 'SyncMethod' then Method := M;
      Assert.IsNotNull(Method);
      Assert.AreEqual('smSynchronized', Method.SyncMode);

      // Check constructor
      Method := nil;
      for var M in Doc.Methods do if M.Name = 'Create' then Method := M;
      Assert.IsNotNull(Method);
      Assert.AreEqual(1, Method.Parameters.Count);
